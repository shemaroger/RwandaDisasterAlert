// components/Header.jsx - Updated with improved LanguageSwitcher integration and full i18n
import React, { useState, useEffect, useRef } from 'react';
import { useNavigate } from 'react-router-dom';
import { 
  Bell, Menu, ChevronDown, User, LogOut, 
  Settings as SettingsIcon, Search, 
  Clock, MapPin, Users, X, Shield, AlertTriangle
} from 'lucide-react';
...
const Header = ({ 
  ...
  const handleLogout = async () => {
    try {
      ...
      if (logoutResult && logoutResult.success) {
        ...
        navigate('/login', {
          replace: true,
          state: { message: t('messages.logoutSuccess') || 'You have been signed out successfully.' }
        });
      }
    } catch (error) {
      ...
      navigate('/login', { 
        replace: true,
        state: { message: t('messages.logoutSuccess') || 'You have been signed out.' }
      });
    }
  };
  ...
  const getUserTypeDisplayName = (userType) => {
    // Use translation keys from the updated translation files
    const typeMap = {
      'admin': t('userTypes.administrator') || 'Administrator',
      'village': t('userTypes.village') || 'Village Level',
      'sector': t('userTypes.sector') || 'Sector Level',
      'district': t('userTypes.district') || 'District Level',
      'province': t('userTypes.province') || 'Province Level',
      'national': t('userTypes.national') || 'National Level',
      'citizen': t('userTypes.citizen') || 'Citizen',
      'authority': 'Emergency Authority', // Legacy
      'operator': 'Emergency Operator' // Legacy
    };
    
    return typeMap[userType] || userType || 'User';
  };
  ...
  const getSystemStatus = () => {
    return {
      status: 'operational',
      color: 'bg-green-500',
      text: t('systemStatus.operational') || 'All Systems Operational'
    };
  };
          ...
          {showSearch ? (
            <form onSubmit={handleSearch} className="relative">
              ...
              <input
                ...
                placeholder={t('search') || "Search incidents, alerts, users..."}
            ...
            <div className="text-center">
              <h2 className="text-xl font-bold text-slate-800 capitalize">
                {currentPage?.replace('-', ' ') || t('dashboard') || 'Dashboard'}
              </h2>
              {user?.district && (
                ...
                <div className="max-h-96 overflow-y-auto">
                  {notifications.length > 0 ? (
                    ...
                    <div className="p-8 text-center text-slate-500">
                      ...
                      <p className="text-sm font-medium">{t('messages.noData') || 'No notifications'}</p>
                      <p className="text-xs text-slate-400 mt-1">{systemStatus.text}</p>
                    </div>
                  )}
                </div>
                {notifications.length > 5 && (
                  <div className="p-3 border-t border-slate-200 text-center bg-slate-50 rounded-b-xl">
                    <button className={`text-sm ${theme.primaryText} hover:text-red-800 font-semibold transition-colors`}>
                      {t('actions.view') || 'View'} {notifications.length} {t('notifications') || 'notifications'}
                    </button>
                  </div>
                )}
              </div>
                ...
                <div className="hidden sm:block text-left">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${getStatusColor()}`}>
                      {t(`status.${user?.status}`) || user?.status || t('status.active') || 'Active'}
                    </span>
                  </div>
                </div>
              )}
              ...
              <div className={`absolute right-0 mt-2 ${isMobile ? 'w-72 max-w-[90vw]' : 'w-72'} bg-white rounded-xl shadow-2xl border border-slate-200 z-[9999] backdrop-blur-sm`}>
                      ...
                      {user?.is_verified === false && (
                        <div className="mt-2">
                          <span className="inline-flex items-center px-2 py-1 rounded-lg text-xs font-semibold bg-amber-100 text-amber-700 border border-amber-200">
                            ...
                            {t('status.pendingVerification') || 'Pending Verification'}
                          </span>
                        </div>
                      )}
                    </div>
                ...
                <div className="py-2">
                  <a href="/profile" className="flex items-center px-4 py-3 text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                    ...
                    <span className="font-medium">{t('profile') || 'Profile Settings'}</span>
                  </a>
                  ...
                  <a href="/help" className="flex items-center px-4 py-3 text-sm text-slate-700 hover:bg-slate-100 transition-colors">
                    <AlertTriangle className="h-4 w-4 mr-3 text-slate-400" />
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div className="flex items-center space-x-1 bg-green-100 rounded-lg px-2 py-1">
                ...
                <span className="text-xs text-green-700 font-medium">{t('online') || 'Online'}</span>
              </div>
            </div>
          </div>
          ...
          {showSearch && (
            <form onSubmit={handleSearch} className="relative mb-3">
              ...
              <input
                ...
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder={t('search') || "Search incidents, alerts..."}
